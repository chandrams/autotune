...
function validate_em_ab_workflow() {
	...
	test_name_=${FUNCNAME}
	input_json="${TEST_DIR_}/../examples/em-jsontemplates/ABTestingTemplate.json"
#	input_json="${TEST_DIR_}/scripts/em/ab.json"
	...
	list_trial_status "${experiment_name}"
...
#	timeout 120s bash -c 'while [ ${exp_status} != ${expected_exp_status} ]; do sleep 5;  list_trial_status "${experiment_name}";  done'
	...
	validate_exp_result "${experiment_name}" "${trial_num}"
