...
def invoke_bulk(worker_number, resultsdir, test, delay):
    try:
        if test == "time_range_rampup" or test == "no_config_rampup":
            time.sleep(delay)

        stress_log_dir = resultsdir + "/stress_logs"
...
def invoke_bulk_with_time_range(worker_number, resultsdir, start_time, end_time, delay):
    try:
        #time.sleep(delay)
        stress_log_dir = resultsdir + "/stress_logs"
        os.makedirs(stress_log_dir, exist_ok=True)
...
def parallel_requests_to_bulk(workers, resultsdir, test, interval_seconds):
    results = []
    with ThreadPoolExecutor(max_workers=workers) as executor:
        # Submit all the tasks to the executor
        futures = [
            executor.submit(invoke_bulk, worker_number, resultsdir, test, delay=worker_number * interval_seconds)
...
def parallel_requests_with_time_range_split(workers, resultsdir, initial_end_time, interval_hours, days_of_res, test, interval_seconds):
    results = []
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for worker_number in range(1, max_workers+1):
                ...
                executor.submit(invoke_bulk_with_time_range, worker_number, resultsdir, current_start_time, current_end_time, delay=worker_number * interval_seconds)
...
def parallel_requests_with_labels(max_workers, resultsdir, initial_end_time, interval_hours, days_of_res, test, interval_seconds):
    ...
    for org_id in range(1, org_ids):
        for cluster_id in range(1, cluster_ids):
            org_label="org_id"
            org_value = "org_id" + org_id
            cluster_label = "cluster_id"
            cluster_value="cluster_id" + cluster_id
...
if __name__ == '__main__':
    ...
    if test == "time_range" or test == "no_config" or test == "time_range_rampup" or test == "no_config_rampup":
        responses = parallel_requests_to_bulk(max_workers, results_dir, test, rampup_interval_seconds)

    if test == "time_range_split":
        responses = parallel_requests_with_time_range_split(max_workers, results_dir, initial_end_date, interval_hours, days_of_res, test, rampup_interval_seconds)

    if test == "time_range_labels":
